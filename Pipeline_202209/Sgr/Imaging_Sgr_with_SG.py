...
#obs.save_txt('simulated_Sgr.txt') # exports a text file with the visibilities
#obs.save_uvfits('simulated_sgr.uvfits') # exports a UVFITS file modeled on template.UVP
...
parser.add_argument('-i', '--infile',    default="simulated_Sgr_with_SG.uvfits", help="input UVFITS file")
parser.add_argument('-i2', '--infile2',  default="",          help="optional 2nd input file (different band) for imaging")
parser.add_argument('-o', '--outfile',   default='simulated_out_Sgr_with_SG.fits',  help='output FITS image')
parser.add_argument('--savepdf',         default='simulated_out_Sgr_with_SG.pdf',       help='saves image pdf (True or False)?',action='store_true')
parser.add_argument('--imgsum',          default='summary_Sgr_with_SG.pdf',       help='generate image summary pdf',action='store_true')
args = parser.parse_args()



pdfout1 = 'Sgrsource' + '.pdf'
...
prior_fwhm = 50.0*eh.RADPERUAS  # Gaussian prior FWHM (radians)
...
beamparams_with_SG = obs.fit_beam()
#-------------------------------------------------------------------------------
...
caltab = eh.selfcal(obs_sc, imgr.out_last(), sites=['LMT'], method='both', gain_tol=gain_tol,
...
for repeat_selfcal in range(2):
    ...
    caltab = eh.selfcal(obs_sc_init, imgr.out_last(), method='both',
                        sites=['PV','KP','SPART','SMA','JCMT','ALMA','APEX','LMT','SPT','SMT','GLT','Yonsei','NOEMA','AMT','SG'],
...
if args.savepdf:
    ...
    #im_out.display(label_type='scale', beamparams=beamparams_with_SG, export_pdf=pdfout)
